...
export const GROESSEN = {
  kachel: 500, // Kachelfassung, fest quadratisch (500 x 500)
...
export async function erzeugeVarianten(eingabe, zielOrdner, basisname) {
  ...
  // Die Kachel ist fest quadratisch und auf das Motiv zugeschnitten — dieselbe
  // Behandlung wie die Kategorie-Kacheln. Vorher behielt sie das Verhaeltnis des
  // Originals, und der Zuschnitt geschah erst im CSS, immer oben ansetzend. Weil
  // 40 der 50 Aufnahmen hochkant sind, blieb dort nur gut die Haelfte des Bildes
  // stehen, bei den acht Querformaten dagegen fast alles: im Raster wirkten die
  // Gegenstaende unterschiedlich gross. Der kuerzeste Bildrand aller Originale
  // misst 556 px, ein 500er Quadrat wird also nie hochgerechnet.
  await sharp(eingabe)
    .rotate() // EXIF-Ausrichtung anwenden, sonst liegen Hochformate quer
    .resize(GROESSEN.kachel, GROESSEN.kachel, {
      fit: "cover",
      position: sharp.strategy.attention,
