...
// Ersetzt einzelne Betraege aus der Anzeige durch die geprueften Werte aus
// inhalte.mjs. Die uebrige Staffel bleibt dabei an der Anzeige haengen —
// anders als bei `preise`, das die ganze Liste einfriert.
//
// Zurueckgegeben wird auch, welche Korrektur ins Leere lief: Steht die Zeile
// nicht mehr im Anzeigentext, ist die Korrektur veraltet und der Bau sagt es.
function korrigierePreise(preise, korrektur) {
  if (!korrektur) return { preise, ungenutzt: [] };
  const getroffen = new Set();
  const neu = preise.map((p) => {
    const wert = korrektur[p.was];
    if (wert === undefined) return p;
    getroffen.add(p.was);
    return { ...p, betrag: wert };
  });
  return { preise: neu, ungenutzt: Object.keys(korrektur).filter((k) => !getroffen.has(k)) };
}
...
function eintrag(anzeige, redaktion) {
  ...
  const roh = redaktion.preise || zerlegt.preise;
  const { preise, ungenutzt } = korrigierePreise(roh, redaktion.preisKorrektur);
  return {
    preisKorrekturOhneWirkung: ungenutzt,
    slug: anzeige.slug,
...
export function baueAngebote(anzeigen, inhalte) {
  ...
  for (const a of anzeigen) {
    ...
    // Eine Preiskorrektur, die keine Zeile trifft, ist stumm wirkungslos —
    // etwa wenn Robert seine Staffel umbenennt. Dann steht wieder der alte
    // Preis auf der Seite, ohne dass es jemand merkt.
    for (const was of e.preisKorrekturOhneWirkung) {
      warnungen.push(`Preiskorrektur "${was}" bei "${e.titel}" greift nicht — diese Zeile steht nicht mehr im Anzeigentext`);
    }
  }
